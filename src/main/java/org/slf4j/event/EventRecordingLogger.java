...
public class EventRecordingLogger extends LegacyAbstractLogger {
    ...
    Queue<org.slf4j.event.SubstituteLoggingEvent> eventQueue;

    // as an event recording logger we have no choice but to record all events
    final static boolean RECORD_ALL_EVENTS = true;

    public EventRecordingLogger(SubstituteLogger logger, Queue<org.slf4j.event.SubstituteLoggingEvent> eventQueue) {
        this.logger = logger;
    ...
    protected void handleNormalizedLoggingCall(Level level, Marker marker, String msg, Object[] args, Throwable throwable) {
        org.slf4j.event.SubstituteLoggingEvent loggingEvent = new SubstituteLoggingEvent();
